...
class ASPP(nn.Module):
    ...
    def __init__(self, C, depth, num_classes =2, dilation_rates=(1, 2, 3, 5), conv=nn.Conv2d, norm=nn.BatchNorm2d, momentum=0.0003,):
        ...
        self.aspp2 = conv(C, depth, kernel_size=3, stride=1,
                          dilation=dilation_rates[1], padding=dilation_rates[1],
                          bias=False)
        self.aspp3 = conv(C, depth, kernel_size=3, stride=1,
                          dilation=dilation_rates[2], padding=dilation_rates[2],
                          bias=False)
        self.aspp4 = conv(C, depth, kernel_size=3, stride=1,
                          dilation= dilation_rates[3], padding=dilation_rates[3],
                          bias=False)
    ...
    def forward(self, x):
        ...
        x5 = nn.AdaptiveAvgPool2d((x.shape[2], x.shape[3]))(x)
        x5 = self.aspp5(x5)
        x5 = self.aspp5_bn(x5)
        x5 = self.relu(x5)
...
class ASPP_modi(nn.Module):
    def __init__(self,
        ...
        self.map_convs = nn.ModuleList([
            ...
            nn.Conv2d(in_channels, hidden_channels, 3, bias=False, dilation=dilation_rates[3],
        ...
        self.global_pooling_conv1 = nn.Conv2d(in_channels, hidden_channels, 1, bias=False)
        self.global_pooling_bn = norm_act(hidden_channels)
        self.global_pooling_conv2 = nn.Conv2d( hidden_channels,  out_channels, 1, bias=False)

        self.out_bn = norm_act(out_channels)

        self.leak_relu = nn.ReLU()
    ...
    def forward(self, x):
        # Map convolutions
        out_aspp = torch.cat([m(x) for m in self.map_convs], dim=1)
        out_aspp = self.map_bn(out_aspp)
        out_aspp = self.leak_relu(out_aspp)       # add activation layer
        out_aspp = self.aspp_conv(out_aspp)

        # Global pooling
        shape = x.shape
        pool = nn.AdaptiveAvgPool2d((shape[2], shape[3]))(x)  # b 64 1 1
        pool = self.global_pooling_conv1(pool)
        pool = self.global_pooling_bn(pool)
        pool = self.leak_relu(pool)  # add activation layer
        pool = self.global_pooling_conv2(pool)
