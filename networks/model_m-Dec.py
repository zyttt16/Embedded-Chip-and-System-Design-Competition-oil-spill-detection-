...
class Decoder_(nn.Module):
    def __init__(self, config):
        ...
        self.conv_block4 = nn.Sequential(
            ...
            Conv2dReLU(self.channels[0], self.channels[0], kernel_size=3, padding=1, use_batchnorm=True)
        )

        self.conv_block3 = nn.Sequential(
            Conv2dReLU(self.channels[1]*3, self.channels[1], kernel_size=1, use_batchnorm=True),
            Conv2dReLU(self.channels[1], self.channels[1], kernel_size=3, padding=1, use_batchnorm=True)
        )

        self.conv_block2 = nn.Sequential(
            Conv2dReLU(self.channels[2]*4, self.channels[2], kernel_size=1, use_batchnorm=True),
            Conv2dReLU(self.channels[2] , self.channels[2], kernel_size=3,padding=1, use_batchnorm=True)
        )
    ...
    def forward(self, hidden_states, features):
        # 256 768  features 512 32 32 / 256 64 64 / 64 128 128
        ...
        add4 = torch.cat([self.up2(x), features[0]], 1)  # 512*2 32 32
        de4 = self.conv_block4(add4)  # 512 32 32

        up_cv3 = self.up_conv1(de4)  # 2 512->256 32->64   2 256 64 64
        add3 = torch.cat([up_cv3, features[1], self.up_f4_32(features[0])], 1) # 2 256*3 64 64
        de3 = self.conv_block3(add3)  # 2 256 64 64

        up_cv2 = self.up_conv2(de3)  # 2 64 128 128
        add2 = torch.cat([up_cv2, features[2], self.up_f4_64(features[0]), self.up_f3_64(features[1])], 1) # 64*4
        de2 = self.conv_block2(add2)  # 2 64 128 128
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # features 512 32 32 / 256 64 64 / 64 128 12
        x = self.decoder2(x, features)
