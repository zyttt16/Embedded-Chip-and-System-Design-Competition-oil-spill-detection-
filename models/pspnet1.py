...
class PSPNet(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        h = int((x_size[2]) / 8 * self.zoom_factor + 1)
        w = int((x_size[3]) / 8 * self.zoom_factor + 1)
...
if __name__ == '__main__':
    ...
    b, c, h, w = input.size()
    model = PSPNet(layers=50, bins=(1, 2, 3, 6), dropout=0.1, classes=2, zoom_factor=1,
                   use_ppm=True, pretrained=True).cuda()
    model.eval()
    # print(model)
    output = model(input)
